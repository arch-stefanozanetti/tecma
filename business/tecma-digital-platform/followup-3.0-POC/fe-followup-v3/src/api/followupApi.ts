...
export const followupApi = {
  ...
  getRentRevenueSummary: (body: { workspaceId: string; projectIds: string[]; dateFrom?: string; dateTo?: string }) =>
    postJson<{
      data: {
        currency: string;
        computedAt: string;
    ...
    }>("/reports/rent-revenue-summary", body),
  runRentRevenueAiQuery: (body: {
    workspaceId: string;
    projectIds: string[];
    query: string;
    dateFrom?: string;
    ...
    }>("/reports/rent-revenue-ai-query", body),
  ...
  postGa4BigDataAiNarrative: (
    projectId: string,
    body: { workspaceId: string; dateFrom: string; dateTo: string }
  ) =>
    postJson<{ data: { markdown: string; generatedAt: string } }>(
      `/bigdata/projects/${encodeURIComponent(projectId)}/ga4-ai-narrative`,
      body
    ),
  getAuditForEntity: (
    entityType: string,
  ...
  getPriceAvailabilityMatrix: (
    ...
    from: string,
    to: string,
    opts?: { unitIds?: string[]; onlyRent?: boolean }
  ) => {
    const qs = new URLSearchParams();
    qs.set("projectIds", projectIds.join(","));
    qs.set("from", from);
    qs.set("to", to);
    if (opts?.onlyRent) qs.set("onlyRent", "1");
    if (opts?.unitIds?.length) qs.set("unitIds", opts.unitIds.join(","));
    return getJson<{
      units: Array<{
        unitId: string;
        projectId?: string;
    ...
    }>(`/workspaces/${encodeURIComponent(workspaceId)}/price-availability?${qs.toString()}`);
  },
  getRentOpenRequestBadges: (workspaceId: string, projectIds: string[], apartmentIds: string[]) => {
    const qs = new URLSearchParams();
    qs.set("projectIds", projectIds.join(","));
    qs.set("apartmentIds", apartmentIds.join(","));
    return getJson<{
      data: { badges: Record<string, { requestId: string; status: string; clientLabel?: string }> };
    }>(`/workspaces/${encodeURIComponent(workspaceId)}/rent-open-request-badges?${qs.toString()}`);
  },
