...
import { ApartmentsRentAvailabilityCalendar } from "./ApartmentsRentAvailabilityCalendar";
import {
  availabilityInfo,
  formatDate,
  pseudoFloor,
...
export const ApartmentsPage = () => {
  ...
  const canCreateRequests = hasPermission("requests.create");
  ...
  const [apartmentsViewTab, setApartmentsViewTab] = useState<"list" | "calendar">("list");
  const [rentOpenBadges, setRentOpenBadges] = useState<
    Record<string, { requestId: string; status: string; clientLabel?: string }>
  >({});

  const showRentAvailabilityTabs =
    modeFilter === "RENT" && isPriceAvailabilityRelevant(projects, selectedProjectIds);

  useEffect(() => {
    if (modeFilter !== "RENT") setApartmentsViewTab("list");
  }, [modeFilter]);
  ...
  useEffect(() => {
    if (!workspaceId || selectedProjectIds.length === 0 || modeFilter !== "RENT" || apartmentsViewTab !== "list") {
      setRentOpenBadges((prev) => (Object.keys(prev).length === 0 ? prev : {}));
      return;
    }
    const ids = apartments.map((a) => a._id).filter(Boolean);
    if (ids.length === 0) {
      setRentOpenBadges({});
      return;
    }
    let ignore = false;
    followupApi
      .getRentOpenRequestBadges(workspaceId, selectedProjectIds, ids)
      .then((r) => {
        if (!ignore) setRentOpenBadges(r.data?.badges ?? {});
      })
      .catch(() => {
        if (!ignore) setRentOpenBadges({});
      });
    return () => {
      ignore = true;
    };
  }, [workspaceId, selectedProjectIds, modeFilter, apartmentsViewTab, apartments]);

  const handleOpenRentRequest = useCallback(
    (requestId: string) => {
  ...
  const handleNewRentNegotiation = useCallback(
    (payload: { apartmentId: string; projectId: string }) => {
      navigate("/requests", {
        state: {
          openNewRequest: true,
          newRequestPreset: { apartmentId: payload.apartmentId, projectId: payload.projectId, type: "rent" },
        },
      });
    },
    [navigate]
    ...
    <div className="min-h-full bg-app font-body text-foreground">
      <div className="px-5 pb-10 pt-8 lg:px-20">
        <ApartmentsListSection
          ...
          showRentAvailabilityTabs={showRentAvailabilityTabs}
          apartmentsViewTab={apartmentsViewTab}
          onApartmentsViewTabChange={setApartmentsViewTab}
          rentOpenBadges={rentOpenBadges}
          onRentBadgeClick={canReadRequests ? handleOpenRentRequest : undefined}
          canReadRequests={canReadRequests}
          rentCalendarSlot={
            workspaceId ? (
              <ApartmentsRentAvailabilityCalendar
                workspaceId={workspaceId}
                projectIds={selectedProjectIds}
                canCreateRequest={canCreateRequests}
                onOpenRequest={handleOpenRentRequest}
                onNewNegotiation={handleNewRentNegotiation}
              />
            ) : null
          }
        />
