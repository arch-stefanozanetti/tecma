...
const CONNECTOR_PROVIDER_URL: Record<string, string | undefined> = {
  connector_twilio: "https://console.twilio.com",
  connector_microsoft_teams: "https://learn.microsoft.com/microsoftteams/platform/webhooks-and-connectors/how-to/add-incoming-webhook",
  connector_mailchimp: "https://mailchimp.com/help/about-api-keys/",
  connector_activecampaign: "https://help.activecampaign.com/hc/en-us/articles/115001546290",
  connector_stripe: "https://dashboard.stripe.com/apikeys",
  connector_paypal: "https://developer.paypal.com/api/rest/",
  connector_webflow: "https://webflow.com/developers/data/reference/authentication",
  connector_n8n: "https://docs.n8n.io/api/authentication/",
  connector_sumsub: "https://docs.sumsub.com/reference",
  connector_meta_whatsapp: "https://developers.facebook.com/docs/whatsapp/cloud-api/get-started",
};

type ConnectorVerifyView = {
  connected: boolean;
  configured: boolean;
...
function getVerifyFailureMessage(connectorId: string, verify: ConnectorVerifyView): string {
  if (verify.hint && verify.hint.trim()) return verify.hint.trim();
  switch (verify.reasonCode) {
    case "CONFIG_MISSING":
      return "Configurazione mancante: completa i campi richiesti e salva prima della verifica.";
    case "AUTH_INVALID":
      return "Credenziali non valide o scadute: aggiorna token/chiavi dal provider e riprova.";
    case "PROVIDER_UNREACHABLE":
      return "Provider non raggiungibile al momento: verifica rete/VPN o riprova tra qualche minuto.";
    case "VERIFY_PARTIAL":
      return "Connessione parziale: una parte dell'integrazione risponde, completa la configurazione residua.";
    case "VERIFY_FAILED":
      return "Verifica non riuscita: controlla i parametri configurati e riprova.";
    case "CONFIG_INVALID":
      return "Configurazione non valida: correggi il formato dei campi richiesti.";
    default:
      return connectorId === "connector_twilio"
        ? "Connessione non completata. Apri Twilio Console o usa la modalita` avanzata."
  ...
  const verifyTwilioConnection = () => {
    ...
    followupApi
      ...
      .then((r) => {
        ...
        setTwilioDrawerError(getVerifyFailureMessage("connector_twilio", r.verify));
  ...
  const verifyTeamsConnection = () => {
    ...
    followupApi
      ...
      .then((r) => {
        ...
        setTeamsDrawerError(getVerifyFailureMessage("connector_microsoft_teams", r.verify));
  ...
  const runConnectorHealthCheck = (connectorId: string) => {
    ...
    verifyCall()
      .then((res) => {
        ...
        const message = getVerifyFailureMessage(connectorId, res.verify);
        const actionUrl = CONNECTOR_PROVIDER_URL[connectorId];
        toast({
          title: message,
          variant: "error",
          actions: actionUrl ? (
            <Button size="sm" variant="outline" className="min-h-9" onClick={() => openProviderPage(actionUrl)}>
              Apri provider
            </Button>
          ) : undefined,
        });
