...
export type NewRequestLocationPreset = {
  apartmentId: string;
  projectId: string;
  type?: RequestType;
};
...
export const RequestsPage = () => {
  ...
  /** Preset da navigazione (es. Appartamenti → Nuova trattativa); consumato dopo applicazione al form. */
  const pendingNewRequestPresetRef = useRef<NewRequestLocationPreset | null>(null);
  ...
  // Apri drawer dettaglio se arriviamo da scheda cliente / cockpit con openRequestId
  useEffect(() => {
  ...
  // Apri form nuova trattativa da deep link (es. cockpit vuoto, Appartamenti con preset)
  useEffect(() => {
    const st = location.state as {
      openNewRequest?: boolean;
      newRequestPreset?: NewRequestLocationPreset;
    } | null;
    const openNewRequest = st?.openNewRequest === true;
    if (!openNewRequest || !canRequestsCreate) return;
    pendingNewRequestPresetRef.current = st?.newRequestPreset ?? null;
    setNewRequestOpen(true);
    navigate(location.pathname, { replace: true, state: {} });
  }, [location.state, location.pathname, navigate, canRequestsCreate]);

  useEffect(() => {
    if (!newRequestOpen) pendingNewRequestPresetRef.current = null;
  }, [newRequestOpen]);

  useEffect(() => {
    if (!newRequestOpen || !workspaceId || selectedProjectIds.length === 0) return;
    setFormError(null);
    const preset = pendingNewRequestPresetRef.current;
    const initialProject =
      preset?.projectId && selectedProjectIds.includes(preset.projectId)
        ? preset.projectId
        : selectedProjectIds[0] ?? "";
    setFormProjectId(initialProject);
    setFormClientId("");
    if (preset) {
      setFormApartmentId("");
      setFormType((preset.type ?? "rent") as RequestType);
    } else {
      setFormApartmentId("");
      setFormType("sell");
    }
  ...
  useEffect(() => {
    if (!newRequestOpen || !workspaceId || !formProjectId) {
      setApartmentsList([]);
      if (!pendingNewRequestPresetRef.current) setFormApartmentId("");
      return;
    }
    ...
    followupApi.requests
      ...
      .then((r) => {
        ...
        if (preset?.projectId === formProjectId && preset.apartmentId) {
          const found = list.some((a) => a._id === preset.apartmentId);
          setFormApartmentId(found ? preset.apartmentId : "");
          pendingNewRequestPresetRef.current = null;
        } else {
          setFormApartmentId("");
        }
      });
