...
const RENT_REQUEST_STATUS_LABEL: Record<string, string> = {
  new: "Nuova",
...
export interface MatrixUnit {
  unitId: string;
  /** Progetto dell’unità (per deep link nuova trattativa). */
  projectId?: string;
  code: string;
  name: string;
  mode?: string;
  /** Trattativa affitto ancora in pipeline su questa unità. */
  openRentRequest?: { requestId: string; status: string; clientLabel?: string };
}
...
export interface PriceAvailabilityGridProps {
  ...
  compact?: boolean;
  /** Apre la trattativa affitto in corso (badge riga). */
  onOpenRentRequest?: (requestId: string) => void;
  /** Avvia nuova trattativa affitto per l’unità (es. da lista Appartamenti). */
  onNewRentNegotiation?: (unit: MatrixUnit) => void;
}
              ...
              <tr key={u.unitId} className="border-b border-border/50 hover:bg-muted/20">
                ...
                >
                  <div className="flex flex-col gap-1.5">
                    ...
                    {(onNewRentNegotiation || (u.openRentRequest && onOpenRentRequest)) && (
                      <div className="flex flex-wrap items-center gap-1.5">
                        {onNewRentNegotiation && (
                          <Button
                            ...
                            variant="outline"
                            size="sm"
                            className={cn("h-7 shrink-0 px-2 text-[10px]", compact && "h-6 text-[9px]")}
                            onClick={(e) => {
                              e.stopPropagation();
                              onNewRentNegotiation(u);
                            }}
                          >
                            Nuova trattativa
                          </Button>
                        )}
                        {u.openRentRequest && onOpenRentRequest && (
                          <button
                            type="button"
                            className={cn(
                              "max-w-[140px] truncate rounded-full border border-amber-200 bg-amber-50 px-2 py-0.5 text-left text-[10px] font-medium text-amber-900 hover:bg-amber-100",
                              compact && "max-w-[100px] text-[9px]"
                            )}
                            title={
                              u.openRentRequest.clientLabel
                                ? `${RENT_REQUEST_STATUS_LABEL[u.openRentRequest.status] ?? u.openRentRequest.status} · ${u.openRentRequest.clientLabel}`
                                : RENT_REQUEST_STATUS_LABEL[u.openRentRequest.status] ?? u.openRentRequest.status
                            }
                            onClick={(e) => {
                              e.stopPropagation();
                              onOpenRentRequest(u.openRentRequest!.requestId);
                            }}
                          >
                            {RENT_REQUEST_STATUS_LABEL[u.openRentRequest.status] ?? u.openRentRequest.status}
                          </button>
