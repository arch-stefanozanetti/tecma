...
export const SECTION_FEATURE_KEY: Partial<Record<string, FeatureKey>> = {
  ...
  rentRevenue: FEATURE_KEYS.reports,
...
/** True se il progetto è trattato come contesto affitto (mode esplicito o naming legacy). */
export function projectIsRentContext(project: ProjectAccessProject): boolean {
  if (project.mode === "rent") return true;
...
export function isPriceAvailabilityRelevant(
  ...
  return selected.some(projectIsRentContext);
}
