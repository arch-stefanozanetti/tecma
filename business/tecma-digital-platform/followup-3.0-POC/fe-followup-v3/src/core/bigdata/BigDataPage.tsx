...
const ExecutiveMarkdownLazy = lazy(() =>
  import("../executive/ExecutiveMarkdown").then((module) => ({ default: module.ExecutiveMarkdown }))
);
...
export const BigDataPage = () => {
  ...
  const projectMarketingTo = useMemo(() => {
    ...
    return `/projects/${encodeURIComponent(projectId)}?workspaceId=${encodeURIComponent(workspaceId)}#project-marketing-analytics`;
  ...
  return (
    <div className="min-h-full bg-app font-body text-foreground">
      <div className="px-5 pb-10 pt-8 lg:px-20">
        <h1 className="text-2xl font-semibold text-foreground">Analisi marketing avanzate</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          ...
          {shortcuts ? (
            <>
              ...
              >
                Progetti → Analisi marketing
              </Link>
            </>
          ) : (
            <>
              in <span className="font-medium text-foreground">Progetti → Analisi marketing</span>
            </>
          )}
          ; token in{" "}
          {shortcuts ? (
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Elenchi da Google Ads, GA4 e Meta come in Progetti → Analisi marketing. Dopo il salvataggio premi &quot;Aggiorna&quot;.
                </p>
              </div>
              <Button
                type="button"
      ...
      <div className="rounded-lg border border-border bg-card/30 p-4 space-y-3">
        ...
        {aiMarkdown && (
          <div className="prose prose-sm dark:prose-invert max-w-none border-t border-border pt-3">
            <Suspense fallback={<p className="text-sm text-muted-foreground">Caricamento sintesi…</p>}>
              <ExecutiveMarkdownLazy source={aiMarkdown} />
            </Suspense>
          </div>
        )}
      </div>
