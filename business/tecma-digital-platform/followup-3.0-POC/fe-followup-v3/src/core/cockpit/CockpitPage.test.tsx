...
import { CockpitPage, selectCockpitUrgentRequests } from "./CockpitPage";
...
describe("CockpitPage", () => {
  ...
  it("mostra errore entitlement senza CTA configura AI", async () => {
    ...
    expect(screen.queryByRole("button", { name: /Configura AI/i })).not.toBeInTheDocument();
  });
  ...
  it("mostra stato provider assente con CTA verso Workspaces", async () => {
    ...
    expect(screen.getByRole("button", { name: /Collega provider/i })).toBeInTheDocument();
  });
});

describe("selectCockpitUrgentRequests", () => {
  it("include trattative new/contacted ferme da oltre 5 giorni", () => {
    const old = new Date();
    ...
    expect(selectCockpitUrgentRequests(stale, [])).toHaveLength(1);
  });

  it("include preventivo in scadenza entro 2 giorni", () => {
    const exp = new Date();
    ...
    const recent = [
      {
        ...
        quoteExpiryOn: exp.toISOString().slice(0, 10),
        quoteStatus: "sent",
      },
    ];
    expect(selectCockpitUrgentRequests([], recent)).toHaveLength(1);
  });
});
