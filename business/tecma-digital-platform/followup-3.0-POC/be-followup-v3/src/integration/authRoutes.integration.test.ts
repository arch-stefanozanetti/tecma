...
describe("integration: auth HTTP routes", () => {
  ...
  it("POST /auth/login authenticates users stored in legacy `users` collection", async () => {
    const { getDb } = await import("../config/db.js");
    const db = getDb();
    const legacyEmail = "legacy-users-collection@test.local";
    await db.collection("users").deleteMany({ email: legacyEmail });
    await db.collection("users").insertOne({
      email: legacyEmail,
      password: await bcrypt.hash("LegacyPass99", 12),
      role: "admin",
      isDisabled: false,
      status: "active",
      project_ids: ["proj-auth-legacy"]
    });

    const res = await st().post("/v1/auth/login").send({ email: legacyEmail, password: "LegacyPass99" });
    expect(res.status).toBe(200);
    expect(res.body.accessToken).toBeTruthy();
