...
import { createOptionalRedisRateLimitStore } from "./rateLimitRedisStore.js";
...
function withOptionalRedisStore<O extends Record<string, unknown>>(opts: O): O {
  const store = createOptionalRedisRateLimitStore();
...
export const platformApiKeyRateLimiter = rateLimit(
  withOptionalRedisStore({
    ...
    legacyHeaders: false,
    skip: () => skipRateLimit,
    keyGenerator: (req: Request) => req.get("x-api-key") ?? ipKeyGenerator(req.ip ?? "")
  })
);
...
/** Query AI report ricavi affitti: bucket dedicato per utente (fallback IP). */
export const rentRevenueAiQueryRateLimiter = rateLimit(
  withOptionalRedisStore({
    windowMs: 60 * 1000,
    max: 30,
    message: { error: "Troppe query AI ricavi affitti, riprova tra un minuto." },
    standardHeaders: true,
    legacyHeaders: false,
    skip: () => skipRateLimit,
    keyGenerator: (req: Request) => {
      const sub = req.user?.sub;
      if (typeof sub === "string" && sub.length > 0) return `rent-revenue-ai:${sub}`;
      return ipKeyGenerator(req.ip ?? "");
    },
  })
);
