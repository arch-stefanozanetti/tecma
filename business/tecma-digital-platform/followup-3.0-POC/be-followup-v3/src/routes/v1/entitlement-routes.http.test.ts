...
describe("HTTP entitlement enforcement", () => {
  ...
  it("GET /v1/workspaces/:workspaceId/connectors/n8n/config → 403 se workspace non accessibile", async () => {
    canAccess.mockResolvedValue(false);
    const token = makeToken();
    const res = await st()
      .get("/v1/workspaces/ws-no-access/connectors/n8n/config")
      .set("Authorization", `Bearer ${token}`);
    expect(res.status).toBe(403);
    expect(res.body.error).toBe("Accesso al workspace non consentito");
  });
});
