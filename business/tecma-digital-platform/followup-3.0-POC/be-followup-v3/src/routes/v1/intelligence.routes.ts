...
import { getRentRevenueSummary, runRentRevenueAiQuery } from "../../core/reports/rent-revenue.service.js";
import {
...
import { queryAuditLog, record as recordAuditLog } from "../../core/audit/audit-log.service.js";
...
import { rentRevenueAiQueryRateLimiter } from "../rateLimitMiddleware.js";
...
intelligenceRoutes.post(
  "/reports/rent-revenue-summary",
  requirePermission(PERMISSIONS.REPORTS_READ),
  requireCanAccessWorkspace(),
  requireWorkspaceEntitled("reports", workspaceIdFromBodyOrQuery),
  handleAsync((req) => getRentRevenueSummary(req.body))
);

intelligenceRoutes.post(
  "/reports/rent-revenue-ai-query",
  rentRevenueAiQueryRateLimiter,
  requirePermission(PERMISSIONS.REPORTS_READ),
  requireCanAccessWorkspace(),
  requireWorkspaceEntitled("reports", workspaceIdFromBodyOrQuery),
  handleAsync(async (req) => {
    const body = (req.body ?? {}) as Record<string, unknown>;
    const result = await runRentRevenueAiQuery(body);
    const queryText = typeof body.query === "string" ? body.query : "";
    const queryHash = crypto.createHash("sha256").update(queryText, "utf8").digest("hex");
    await recordAuditLog({
      action: "security.reports.rent_revenue.ai_query",
      workspaceId: result.data.scope.workspaceId,
      entityType: "rent_revenue_ai_query",
      entityId: queryHash.slice(0, 24),
      actor: { type: "user", userId: req.user?.sub, email: req.user?.email },
      payload: {
        projectIds: result.data.scope.projectIds,
        dateFrom: result.data.scope.dateFrom,
        dateTo: result.data.scope.dateTo,
        queryHash,
      },
      ip: req.ip,
    });
