...
const mocks = vi.hoisted(() => {
  ...
  const requestsCountDocumentsMock = vi.fn(() => Promise.resolve(0));
  const apartmentLocksCountDocumentsMock = vi.fn(() => Promise.resolve(0));
  const projectWorkflowSettingsCountDocumentsMock = vi.fn(() => Promise.resolve(0));
  const projectWorkflowSettingsFindOneMock = vi.fn(() => Promise.resolve(null));
  ...
  return {
    ...
    apartmentLocksCountDocumentsMock,
    projectWorkflowSettingsCountDocumentsMock,
    projectWorkflowSettingsFindOneMock,
    workflowsCollection,
    statesCollection,
...
vi.mock("../../config/db.js", () => ({
  getDb: () => ({
    collection: (name: string) => {
      ...
      if (name === "tz_requests") return { countDocuments: mocks.requestsCountDocumentsMock };
      if (name === "tz_apartment_locks") return { countDocuments: mocks.apartmentLocksCountDocumentsMock };
      if (name === "tz_project_workflow_settings") {
        return {
          findOne: mocks.projectWorkflowSettingsFindOneMock,
          countDocuments: mocks.projectWorkflowSettingsCountDocumentsMock,
        };
      }
...
describe("workflow-engine.service", () => {
  ...
  it("deleteWorkflow throws 409 when workflow is referenced by requests", async () => {
    ...
    mocks.requestsCountDocumentsMock.mockResolvedValueOnce(3);
  ...
  it("deleteWorkflowState throws 409 when state has runtime references", async () => {
    const stateId = new ObjectId();
    mocks.requestsCountDocumentsMock.mockResolvedValueOnce(5);
