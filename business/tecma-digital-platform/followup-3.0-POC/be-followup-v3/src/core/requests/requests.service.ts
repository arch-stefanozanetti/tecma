...
/**
 * Ultima trattativa affitto "aperta" per appartamento (per badge su lista/calendario).
 * Max 200 apartmentId per chiamata.
 */
export const getOpenRentRequestBadgesByApartmentIds = async (
  ...
  if (validIds.length > 200) return {};

  const db = getDb();
  const collection = db.collection(COLLECTION_NAME);
  const apartmentMatchVariants = [...new Set(validIds.flatMap((id) => [id, new ObjectId(id)]))];
  ...
  const docs = await collection
    .find({
      ...
      type: "rent",
      status: { $nin: ["won", "lost"] },
      apartmentId: { $in: apartmentMatchVariants },
    })
    .sort({ updatedAt: -1 })
    .toArray();

  const pickedByApartment = new Map<string, Record<string, unknown>>();
  for (const doc of docs) {
    const raw = doc.apartmentId;
    const aid =
      typeof raw === "object" && raw instanceof ObjectId
        ? raw.toHexString()
        : raw != null
          ? String(raw)
          : "";
    if (!aid || pickedByApartment.has(aid)) continue;
    pickedByApartment.set(aid, doc as Record<string, unknown>);
  }

  const clientIds = [...pickedByApartment.values()]
    .map((d) => String(d.clientId ?? ""))
    .filter(Boolean);
  const clientNames = await batchLoadClientNames(db, clientIds);
  ...
  for (const [aptId, doc] of pickedByApartment) {
    ...
    out[aptId] = {
      requestId: row._id,
      status: row.status,
      clientLabel: cid ? clientNames[cid] : undefined,
    };
  }
  return out;
};
