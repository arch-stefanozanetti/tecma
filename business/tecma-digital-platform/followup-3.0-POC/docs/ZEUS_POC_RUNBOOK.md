...
2. **Variabili ambiente backend** utili:
   ...
   - `ZEUS_SIP_ALLOWED_CIDRS` — opzionale allowlist IP/CIDR (IPv4/IPv6) per webhook SIP.
...
| SIP Voice (Track B) | `POST` | `https://<BACKEND>/v1/workspaces/<WORKSPACE_ID>/zeus/webhooks/sip/voice` — payload JSON con `transcript`/`text`, auth via secret + anti-replay (`x-zeus-sip-ts`, `x-zeus-sip-signature`) |
...
curl -X POST "https://<BACKEND>/v1/workspaces/<WORKSPACE_ID>/zeus/webhooks/sip/voice" \
  ...
  -H "x-zeus-sip-ts: <UNIX_TS_MS>" \
  -H "x-zeus-sip-signature: <HMAC_SHA256_HEX>" \
...
- Verificare firma anti-replay su ogni request SIP (`x-zeus-sip-ts` + `x-zeus-sip-signature`, finestra max 5 minuti).
