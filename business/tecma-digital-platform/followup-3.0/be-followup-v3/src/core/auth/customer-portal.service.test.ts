...
const mocks = vi.hoisted(() => {
  ...
  return { insertOne, findOne, updateOne, find, sort, limit, toArray };
});
...
vi.mock("../../config/db.js", () => ({
  getDb: () => ({
    collection: (name: string) => {
      ...
      if (name === "tz_portal_sessions") return { insertOne: mocks.insertOne, findOne: mocks.findOne };
