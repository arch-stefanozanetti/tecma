# Experimental Editor (`/experimental`)
...
L'esperienza utente e embedded-only nella shell di Followup: nessuna apertura in nuova scheda.

## Struttura

- Host app Followup: `fe-followup-v3`
- App sperimentale: `experimental/pascal-editor` (Next.js)
- Hub interno Followup: `/experimental` (catalogo tile)
- Dettaglio esperimento: `/experimental/:experimentId` (es. `/experimental/pascal-editor`)
- Path tecnico editor separato: `/experimental/editor` (reverse proxy interno)
...
- I dettagli `/experimental/:experimentId` sono anch'essi `tecmaAdminOnly`.
