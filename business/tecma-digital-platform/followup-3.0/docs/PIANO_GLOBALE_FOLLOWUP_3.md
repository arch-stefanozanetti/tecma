...
| `csv-mapping` | CSV legacy → mapping cliente / appartamento / quote → dominio operativo + API/UI | **[~] Ciclo 3 (2026-04-14):** mapping **quote** `asset.quotes` → `tz_quotes` documentato in [FASE1_CSV_MAPPING.md](./deliverables/FASE1_CSV_MAPPING.md); migrazione pilota arricchita `totalPrice`; `POST /v1/quotes/query` + OpenAPI. Restano matrici cliente/appartamento/CSV export dove non coperte da Mongo read-only. |
...
| `visual-parity` | Parità visiva rispetto al design system ITD (ref. repo `tecma-fe-apps/fe-tecma-itd`) | Checklist: [deliverables/FASE8_VISUAL_PARITY.md](./deliverables/FASE8_VISUAL_PARITY.md) |
| `ux-mobile` | UX mobile: checklist per pagina, implementazione incrementale | |
| `refactor-api-layer` | Refactor client API per dominio (FE); pagine hub a fasi, PR separate | |
...
**UX non Tecma:** vetrina (benefici, CTA, footnote su card se modulo disattivo) in Integrazioni; tab API con banner se `publicApi` assente. *Rifiniture copy/link CRM ancora consigliate.*

**UX Tecma Admin:** pagina `/tecma/entitlements` (selettore workspace, stato, **note**, PATCH + audit). *Completata baseline; eventuali campi extra commerciali = evoluzione.*

**Milestone:** ~~(1) modello DB~~ ~~(2) policy engine (minimo)~~ ~~(3) endpoint governance (minimo)~~ ~~(4) vetrina FE (minimo)~~ ~~(5) console Tecma (minimo)~~ ~~(6) audit (PATCH)~~ (7) rollout graduale commerciale + **ogni nuova route a pagamento** da elencare in matrice entitlement.
