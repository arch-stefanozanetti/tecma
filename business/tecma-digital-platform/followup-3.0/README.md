...
- **Inviti email (pagina Utenti):** servono **`EMAIL_TRANSPORT=smtp`**, **`SES_SMTP_USER`** / **`SES_SMTP_PASS`** e **`EMAIL_FROM`** verificato in SES. Il link “Imposta password” usa **l’URL del browser** da cui inviti (`Origin` o `appPublicUrl` dal FE): in locale → `http://localhost:5177`, su Render → il tuo `*.onrender.com`. **`APP_PUBLIC_URL`** è solo **fallback** (es. script senza browser). Domini custom: **`INVITE_LINK_ALLOWED_HOSTS`** (host separati da virgola). Senza SMTP, `POST /v1/users` → **503**. Solo dev senza SES: **`INVITE_ALLOW_MOCK_EMAIL=true`** + **`EMAIL_TRANSPORT=mock`**.
- **Env**: `APP_ENV` opzionale (default `dev-1`), usato per log/label. Deploy = stesso codice, variabili diverse per ambiente.
- **Auth (Wave 6):** JWT access token + refresh token. Route protette da middleware `requireAuth` (tranne health, openapi, login, sso-exchange, refresh, logout). Refresh token salvati in collection `tz_authSessions`; variabile `AUTH_REFRESH_EXPIRES_DAYS` (default 7). Eventi di login, logout e sso_exchange sono registrati in `tz_authEvents` per audit; policy di retention e uso: [docs/AUTH_AUDIT_POLICY.md](docs/AUTH_AUDIT_POLICY.md). Login e sso-exchange restituiscono `accessToken`, `refreshToken`, `user`; il frontend su 401 tenta il refresh e ritenta la richiesta, altrimenti redirect a login. **In modalità gateway + BSS auth** (`VITE_USE_BSS_AUTH=true` nel FE) le chiamate di login, me e refresh vanno al **gateway (backend BSS)**, non a be-followup-v3.
- **API Gateway**: le API sono pensate per essere esposte come path generici in **TECMA-BSS** (aws-api-gateway). Addizioni OpenAPI da mergiare nello spec BSS: [docs/openapi-tecma-bss-additions.yaml](docs/openapi-tecma-bss-additions.yaml) (integrazione `http://${stageVariables.url}/v1/...`). Piano di unificazione con aws-api-gateway (test, Postman, doc, FE): [docs/plans/2025-03-07-unificazione-api-aws-gateway-followup.md](docs/plans/2025-03-07-unificazione-api-aws-gateway-followup.md).
