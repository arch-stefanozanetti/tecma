...
**Deploy Render:** i servizi `followup-3-be` / `followup-3-fe` su onrender.com compilano **solo** il **progetto** in `followup-3.0-POC/` (vedi [`render.yaml`](../../../render.yaml)). La directory `followup-3.0/` è un **progetto separato** nello stesso monorepo Git; non c’è un flusso di “porting” obbligato verso la POC. Convivenza e confini: [`FOLLOWUP_MONOREPO_LAYOUT.md`](../FOLLOWUP_MONOREPO_LAYOUT.md).
...
- **API Gateway**: le API sono pensate per essere esposte come path generici in **TECMA-BSS** (aws-api-gateway). Addizioni OpenAPI da mergiare nello spec BSS: [docs/openapi-tecma-bss-additions.yaml](docs/openapi-tecma-bss-additions.yaml) (integrazione `http://${stageVariables.url}/v1/...`). Contesto e prossimi passi: [docs/PIANO_GLOBALE_FOLLOWUP_3.md](docs/PIANO_GLOBALE_FOLLOWUP_3.md) §14.1 e [docs/AUTH_AND_TECMA_BSS_API_REPORT.md](docs/AUTH_AND_TECMA_BSS_API_REPORT.md).
