#!/usr/bin/env node
/**
 * Migrazione idempotente della collection `tz_users` sul DB **test-zanetti**:
 * - email trim + lowercase
 * - password (bcrypt $2*) → passwordHash + $unset password
 * - status da isDisabled / status legacy
 * - $unset isDisabled
 *
 * Default: dry-run. Esegui modifiche con `--apply`.
 */
import { MongoClient } from 'mongodb';

const DB_NAME = 'test-zanetti';
...
function assertDbName(db) {
  if (db.databaseName !== DB_NAME) {
    console.error(`Refused: connected database is "${db.databaseName}", expected "${DB_NAME}".`);
    process.exit(1);
  }
}
...
async function main() {
  ...
  try {
    ...
    const db = client.db(process.env.MONGO_DB_NAME ?? DB_NAME);
    assertDbName(db);
