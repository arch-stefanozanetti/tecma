...
describe('workspaces permissions integration', () => {
  ...
  it('enforces requireAnyPermission for non-admin users', async () => {
    const adminToken = await loginToken('admin-perm@tecma.test');
    const userToken = await loginToken('viewer-perm@tecma.test');
  ...
  it('espone audit events solo a Tecma SuperAdmin con filtri platform', async () => {
    ...
    const allowed = await app.inject({
      method: 'GET',
      url: '/v1/admin/audit-events?workspaceId=ws-audit-test&eventType=admin.audit.test&perPage=1',
      headers: authHeaders(adminToken),
    });
    expect(allowed.statusCode).toBe(200);
    expect(allowed.json().paginationInfo).toMatchObject({
      totalDocs: 2,
      page: 1,
      perPage: 1,
      totalPages: 2,
