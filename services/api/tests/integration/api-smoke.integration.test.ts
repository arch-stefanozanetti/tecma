...
import { SMOKE_API_KEY, seedSmokeFixture } from '../helpers/smokeFixture.js';

const API_KEY = SMOKE_API_KEY;
...
describe('api smoke integration', () => {
  beforeAll(async () => {
    ...
    ({ demoUserId } = await seedSmokeFixture(app.mongoDb));
  });
  ...
  it('login then auth/me, workspaces, projects, session/preferences', async () => {
    ...
    const cookieHeader = Array.isArray(loginSetCookie)
      ? loginSetCookie[0]
    ...
    const refreshToken = cookieHeader
      ...
      ?.slice('followup_refresh_token='.length);
    ...
    const refresh = await app.inject({
      ...
      headers: {
        'x-api-key': API_KEY,
        cookie: `followup_refresh_token=${refreshToken!}`,
      },
    });
