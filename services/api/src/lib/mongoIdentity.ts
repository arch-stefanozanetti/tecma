...
export const INACTIVE_MEMBERSHIP_STATUSES = ['deleted', 'deactivated', 'suspended'] as const;
export const INACTIVE_RESOURCE_STATUSES = [
  'deleted',
  'deactivated',
  'suspended',
  'archived',
] as const;
export const INACTIVE_ACCESS_STATUSES = [
  'deleted',
  'deactivated',
  'suspended',
  'archived',
...
export const activeMembershipStatusFilter = (): Record<string, unknown> => ({
  status: { $nin: [...INACTIVE_MEMBERSHIP_STATUSES] },
});

export const activeResourceStatusFilter = (): Record<string, unknown> => ({
  status: { $nin: [...INACTIVE_RESOURCE_STATUSES] },
});

export const activeAccessStatusFilter = (): Record<string, unknown> => ({
  status: { $nin: [...INACTIVE_ACCESS_STATUSES] },
});
...
/**
 * Varianti stringa/ObjectId per `_id` di documenti Mongo (querystring/API vs BSON legacy).
 * Evita 404 “workspace not found” quando il client invia l’hex string e il DB ha `ObjectId`.
 */
export function mongoPrimaryKeyFilter(id: string): Record<string, unknown> {
  const t = id.trim();
  const strings: string[] = [];
  const oids: ObjectId[] = [];
  const seenStr = new Set<string>();
